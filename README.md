...
> 처음 만드니 코드가 돌아가는걸 목표로 무식하게 만듭니다.    
...
#### 웹
위키피디아 검색결과  
> 월드 와이드 웹(World Wide Web, WWW, W3)은 인터넷에 연결된 컴퓨터들을 통해   
> 사람들이 정보를 공유할 수 있는 전 세계적인 정보 공간을 말한다. 간단히 웹(Web)이라 부르는 경우가 많다.   
...
#### URL
웹에서 웹 페이지를 정의하고 접근하기 위해 URL 을 사용한다. URL 의 일반적인 형식은
...
통신 방법이 여러가지 있지만 레일즈에선 `get` `post` `patch` `delete`을 사용한다.    
`get`은 URL창에 정보를 보여주며 통신하고  
`post`는 정보를 숨기며 통신한다.  
이해가 어렵다면 `get`과 `post`가 존재한다는 것만 알아둡니다.    
...
이해가 불가능하다..  
`REST`는 소프트웨어 구조로 웹에서 효율적으로 통신하기 위한 방법으로 정리합니다.  
이번 세션에는 `REST`를 적용해 라우터를 사용하긴 힘듭니다.  
다음 세션에 사옹해봅니다.  
